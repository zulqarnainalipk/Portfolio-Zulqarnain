...
// Animated skill grid component (a more modern way to visualize skills)
const SkillGrid: React.FC<{ skills: string[], colors: string[] }> = ({ skills, colors }) => {
  const { ref, inView } = useInView({ triggerOnce: true, threshold: 0.1 });
    ...
    >
      {skills.map((skill, i) => {
        ...
        return (
          <motion.div
            key={i}
            className="bg-secondary/30 backdrop-blur-sm rounded-lg p-3 flex items-center justify-center text-center h-24"
            variants={{
            ...
            whileHover={{
              y: -5,
              boxShadow: `0 10px 20px -5px ${colors[colorIndex]}40`,
              borderColor: colors[colorIndex],
              scale: 1.05,
              transition: { duration: 0.2 }
            }}
            style={{
              border: `1px solid ${colors[colorIndex]}30`,
...
// Main enhanced 2D Skills Visualization Component (No 3D!)
const SkillsVisualization2DEnhanced: React.FC = () => {
  ...
  // Get all colors for the skill grid
  const allColors = enhancedSkillCategories.map(cat => cat.color);
  ...
  return (
    <div className="w-full space-y-12">
      ...
      >
        <h3 className="text-xl font-semibold text-primary mb-4">All Skills at a Glance</h3>
        <SkillGrid skills={allSkills} colors={allColors} />
      </motion.div>
    </div>
  );
};

export default SkillsVisualization2DEnhanced;